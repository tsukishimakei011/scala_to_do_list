...
object DateUtil {
  ...
  implicit class DateFormater (val date : LocalDate){ //auto-convert all local date into date formatter object
  ...
  implicit class StringFormater (val data : String) {
    ...
    def isValid : Boolean = {
      data.parseLocalDate != null  // best way is use option, no null
    }
  }
}
